function param_nodes=fit_PPL_model_2cov(DATA,GRID,CV,Nboot)
...
if Nboot==1
    % fit model
    paramhat=fmincon(@(p)gp_negloglike_PPL2cov(p,lambda_opt,const_xi,DATA,GRID),param0,[],[],[],[],param_LB,param_UB,[],CV.options);
    ...
    for i=1:Nboot
        ...
        paramhat=fmincon(@(p)gp_negloglike_PPL2cov(p,lambda_opt,const_xi,DATAboot,GRID),param0,[],[],[],[],param_LB,param_UB,[],CV.options);
